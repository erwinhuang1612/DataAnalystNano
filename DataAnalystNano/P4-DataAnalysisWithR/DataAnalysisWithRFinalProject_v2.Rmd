---
title: "Udacity: Data Analysis With R Final Project"
author: "Erwin"
date: "6 May 2017"
output:
...
### Prosper Peer-to-peer Lending Platform Dataset
![ ](http://mms.businesswire.com/media/20160314005400/en/512432/5/Prosper_NewLogo_300x150.jpg)

This dataset contains a list of loans of [Prosper Peer-to-peer Lending Platform](https://www.prosper.com/) from 2005 to 2016 with 81 variables in total. I have chosen this dataset because it contains various variables related to peer to peer lending that can be explored to understand the basic purpose of getting P2P loan, overall customer behaviour in the P2P platform.
...
```{r,Subset useful variables, echo=FALSE, message = FALSE, warning=FALSE}
...
```{r, Stated Monthly Income, echo=FALSE, message=FALSE , warning=FALSE, fig.width=12, fig.height=6}
...
histo <- ggplot(aes(x=StatedMonthlyIncome), data=loan) + 
...
box <- ggplot(aes(x=1,y=StatedMonthlyIncome), data=loan) +
    scale_y_continuous(breaks=seq(0,600000,2000),limits=c(0,27000)) + 
    geom_boxplot(fill='#05DBF2', 
                   color='black', 
                  alpha= 0.2,
                   size = 1) +
...
```{r, Loan Original Amount 1, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6}
histo <- ggplot(aes(x=LoanOriginalAmount), data=loan) + 
    geom_histogram(fill='#05DBF2', 
                   color='black', 
                   binwidth=500) +
...
box <- ggplot(aes(x=1,y=LoanOriginalAmount), data=loan) +
    scale_y_continuous(breaks=seq(0,27000,2500),limits=c(0,27000)) + 
    geom_boxplot(fill='#05DBF2', 
                   color='black', 
                  alpha= 0.2,
                   size = 1) +
...
ggplot(data=loan, aes(x=EmploymentStatusDuration)) +
    ...
    labs(title = "Employment Status Duration",
       x= "Employment Status Duration (months)")
...
```{r,  Debt to Income Ratio, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6}
histo <- ggplot(aes(x=DebtToIncomeRatio), data=loan) + 
    geom_histogram(fill='#05DBF2', 
                   color='black', 
                   binwidth=0.05) +
...
box <- ggplot(aes(x=1,y=DebtToIncomeRatio), data=loan) +
...
```{r, Monthly Debt, echo=FALSE, message=FALSE , warning=FALSE, fig.width=12, fig.height=6}
...
box <- ggplot(aes(x=1,y=TotalMonthlyDebt), data=loan) +
...
```{r,  Borrower Interest Rate, echo=FALSE, message=FALSE , warning=FALSE, fig.width=12, fig.height=6}
...
histo <- ggplot(aes(x=BorrowerRate), data=loan) + 
    geom_histogram(fill='#05DBF2', 
                   color='black', 
                   binwidth=0.005) +
...
box <- ggplot(aes(x=1,y=BorrowerRate), data=loan) +
...
```{r, correlation matrix , echo=FALSE, message=FALSE , warning=FALSE, fig.width=12,figh.height=10}
...
- IncomeRange_reordered and LoanOriginalAmount
- ProsperRating and BorrowerRate
- BorrowerRate and CreditRating
...
ggplot(data=loan, aes(x=factor(CreditScoreRangeLower),y=BorrowerRate))+
  geom_boxplot() +
...
```{r, income vs total debt amount, echo=FALSE, message=FALSE , warning=FALSE}
...
```{r,  loan amount vs interest rate, echo=FALSE, message=FALSE , warning=FALSE }
...
```{r, loan amount vs credit score, echo=FALSE, message=FALSE , warning=FALSE}
...
```{r, In depth Analysis on Borrower Rate, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 13, fig.height = 6}
ggplot(data=loan, aes(x=BorrowerRate)) + 
  geom_histogram(binwidth=0.01) + 
  scale_x_continuous(breaks=seq(0.3,0.35,0.01),limits=c(0.3,0.35)) +
  facet_wrap(~listingCategory) +
  scale_fill_discrete(name="Loan Category") + 
  labs(title="Count of Borrower Rate by Listing Category",
         x='Borrower Rate')
```

```{r, In depth Analysis on Borrower Rate Spike, echo=FALSE, message=FALSE , warning=FALSE, fig.width = 13, fig.height = 6}
ggplot(data=loan, aes(x=BorrowerRate)) + 
  geom_histogram(binwidth=0.01) + 
  scale_x_continuous(breaks=seq(0.3,0.35,0.01),limits=c(0.3,0.35)) +
  facet_wrap(~LoanYear) +
  theme(axis.text.x = element_text(angle=45,size=8)) + 
    labs(title="Count of Borrower Rate by Loan Year",
         x='Borrower Rate')
```
```{r, Analysis on Borrower Rate, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6}
ggplot(data=subset(loan, loan$CreditScoreRangeLower!=0), 
       aes(x=BorrowerRate)) + 
  geom_histogram(binwidth=0.01) + 
  scale_x_continuous(breaks=seq(0.3,0.35,0.01),limits=c(0.3,0.35)) +
  facet_wrap(~CreditScoreRangeLower) + 
    labs(title="Count of Borrower Rate by Credit Score",
         x='Borrower Rate')
```
...
```{r,  loan amt over date by cat, echo=FALSE, message=FALSE , warning=FALSE, fig.width=12, fig.height=6}
...
```{r, Loan Listing Category and Loan Amount, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 10,}
ggplot(data=loan,
       aes(x=reorder(listingCategory,-LoanOriginalAmount,median),
           y=LoanOriginalAmount)) + 
      geom_boxplot() + 
      scale_fill_discrete(name="Loan Listing Category")+
      theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
      labs(title="Loan amount by Loan Month",
      x='Loan Month',
      y='Loan amount ($)')+
...
```{r, Borrowers segmentation, echo=FALSE, message=FALSE , warning=FALSE,, fig.width=12, fig.height=6}
...
```{r,  Monthly Debt to Income Ratio by Loan Year Faceted by Income Range , echo=FALSE, warning=FALSE, message=FALSE, fig.width = 13, fig.height = 6}

ggplot(data=subset(loan, loan$IncomeRange_reordered != '$0' & loan$IncomeRange_reordered != 'Not displayed'), 
       aes(x=format(as.Date(LoanOriginationDate ),"%Y"), y= MonthlyLoanPayment/StatedMonthlyIncome, 
           fill=IncomeRange_reordered)) +
  geom_boxplot() +ylim(0, 0.1) + 
  theme(axis.text.x = element_text(angle=45, vjust=1, size=8)) +
  labs(title="Monthly Loan Payment to Income Ratio by Loan Year by Income Range",
  x='Loan Year',
...
```{r,  Debt to Income Ratio By Loan Year Faceted by Income Range , echo=FALSE, warning=FALSE, message=FALSE, fig.width = 13, fig.height = 6}
...
```{r, Loan Amount by Year and Income Range , echo=FALSE, message=FALSE , warning=FALSE, fig.width = 13, fig.height = 6}
...
ggplot(data=subset(YearlyIncomeAndLoan, YearlyIncomeAndLoan$IncomeRange_reordered != '$0'),
  ...
  labs(title="Loan Amount by Year Faceted by Income Range",
  x='Year ',
  y='Sum of Loan Amount ($)')+
  facet_wrap(~IncomeRange_reordered) + 
...
### Plot One
```{r, Distribution of loan count by Year, echo=FALSE,  message=FALSE, warning=FALSE, fig.width = 10, fig.height = 10}
...
box1 <- ggplot(data=loan, aes(x=LoanYear,y=LoanOriginalAmount)) + 
  geom_boxplot() +
  labs(title="Loan Amount by Loan Year",
  x='Loan Year',
  y='Loan Amount ($)')+
  stat_summary(fun.y = "mean", 
               geom = "point", 
...
box2 <- ggplot(data=loan, aes(x=LoanYear,y=sum(LoanOriginalAmount))) + 
  geom_bar(stat = "identity") +
  labs(title="Sum of Loan Amount by Loan Year",
  x='Loan Year',
  y='Sum of Loan Amount ($)')
...
```{r,Borrower Rate and Loan Amount by Year, echo=FALSE, message= FALSE, warning=FALSE, fig.width = 13, fig.height = 6}
# ggplot(data=subset(loan, loan$CreditScoreRangeLower > 650), 
...
plot1 <- ggplot(data=subset(loan, loan$CreditScoreRangeLower > 650), 
              aes(x=factor(CreditScoreRangeLower))) +
  geom_bar() +
  facet_wrap(~LoanYear) +
  labs(title="Credit Score Range and Number of Loans by Year",
  x='Credit Score Range',
  y='Number of loans') + 
  theme(axis.text.x = element_text(angle=45, vjust=1, size=7))

# plot2 <- ggplot(data=subset(loan, loan$CreditScoreRangeLower > 700), 
#               aes(x=factor(CreditScoreRangeLower),y=mean(BorrowerRate))) +
#   stat_summary(fun.y="mean", geom="bar") +
#   facet_wrap(~LoanYear) +
#   labs(title="Credit Score Range and Average Borrower Rate by Year",
#   x='Credit Score Range',
#   y='Average Borrower Rate') + 
#   theme(axis.text.x = element_text(angle=45, vjust=1, size=7))

plot2 <- ggplot(data=subset(loan, loan$CreditScoreRangeLower >= 600),
              aes(x=factor(CreditScoreRangeLower),y=BorrowerRate)) +
  geom_boxplot() +
  facet_wrap(~LoanYear) +
  labs(title="Credit Score Range and Borrower Rate by Year",
  x='Credit Score Range',
  y='Borrower Rate') +
  theme(axis.text.x = element_text(angle=45, vjust=1, size=7))+
  stat_summary(fun.y = "mean",
...
### Plot Three
```{r, loan amount by loan category and year, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 13, fig.height = 6}
...
ggplot(data=loanByYear, aes(x=LoanYear, y=DollarTotal/1000000)) +
  geom_bar(stat='identity') +
  facet_wrap(~listingCategory) +
  scale_fill_discrete(name="Loan Listing CategoryCategory") +
